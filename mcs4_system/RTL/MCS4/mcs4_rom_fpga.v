...
// MCS-4 Project
//-----------------------------------------------------------
// File Name   : mcs4_rom_fpga.v
// Description : MCS-4 ROM Chip (i4001 x 16chips) for FPGA
//-----------------------------------------------------------
// History :
...
//
assign rom_addr = (state[`A3] & (CM_N == 1'b0))? {DATA_I, rom_addr_lsb} : 12'h000;
//
//always @(posedge CLK)
//begin
//    if (state[`A3] & (CM_N == 1'b0))
//        rom_rdata <= rom[rom_addr];
//end
//
ROM4KB_SP U_MCS4_ROM
(
    .address (rom_addr),
    .clock   (CLK),
    .rden    (state[`A3] & (CM_N == 1'b0)),
    .q       (rom_rdata)
);
//
